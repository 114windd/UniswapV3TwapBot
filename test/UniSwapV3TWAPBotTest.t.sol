...
contract UniswapV3TWAPBotTest is Test {
     ...
     */
    function testFuzz_ExecuteSliceWithVariousStates(
        uint8 userSeed,
        uint8 tokenSeed,
        uint256 totalAmount,
        uint8 slicesToExecute,
        uint256 timeSkip
    ) public {
        // Bound inputs
        userSeed = uint8(bound(userSeed, 0, 2));
        tokenSeed = uint8(bound(tokenSeed, 0, 2));
        slicesToExecute = uint8(bound(slicesToExecute, 0, 5)); // Execute 0-5 slices first
        timeSkip = bound(timeSkip, 0, TEST_INTERVAL * 2); // 0 to 2 intervals extra time

        // Select user and tokens
        address user;
        if (userSeed == 0) user = user1;
        else if (userSeed == 1) user = user2;
        else user = user3;
        ...
        if (tokenSeed == 0) {
            tokenIn = address(tokenUSDC);
            tokenOut = address(tokenWETH);
            maxAmount = 1_000 * 10 ** USDC_DECIMALS; // Reduced from 5k to 1k
        } else if (tokenSeed == 1) {
            tokenIn = address(tokenWETH);
            tokenOut = address(tokenDAI);
            maxAmount = 1 * 10 ** WETH_DECIMALS; // Reduced from 5 to 1
        } else {
            tokenIn = address(tokenDAI);
            tokenOut = address(tokenUSDC);
            maxAmount = 2_000 * 10 ** DAI_DECIMALS; // Reduced from 20k to 2k
        }

        // Bound total amount to ensure router has sufficient liquidity
        totalAmount = bound(totalAmount, 120, maxAmount); // Minimum 120 units for 12 slices (10 each)
        ...
        // Execute some slices first to test different order states
        uint256 maxPossibleSlices = _getExpectedSliceCount();
        if (slicesToExecute > maxPossibleSlices) {
            slicesToExecute = uint8(maxPossibleSlices - 1); // Leave at least 1 slice
        }
        ...
        // Get state before the test execution
        UniswapV3TWAPBot.Order memory orderBefore = twapBot.getOrder(orderId);
        uint256 userOutBalanceBefore = twapBot.userBalances(user, tokenOut);
        uint256 remainingBefore = twapBot.remainingAmount(orderId);

        // Skip additional time
        vm.warp(block.timestamp + timeSkip);

        // Skip to next valid execution time
        _skipToNextExecution(orderId);

        // Verify we can still execute (not all slices done)
        if (orderBefore.slicesExecuted < maxPossibleSlices) {
            // Execute slice and verify success
            uint256 amountOut = twapBot.executeSlice(orderId);
            ...
            // Get updated state
            UniswapV3TWAPBot.Order memory orderAfter = twapBot.getOrder(
                orderId
            );

            // Verify slice execution incremented
            assertEq(
                orderAfter.slicesExecuted,
                orderBefore.slicesExecuted + 1,
                'Slices executed should increment'
            );

            // Verify total out increased
            assertEq(
                orderAfter.totalOut,
                orderBefore.totalOut + amountOut,
                'Total out should increase by amount out'
            );

            // Verify user balance increased
            uint256 userOutBalanceAfter = twapBot.userBalances(user, tokenOut);
            assertEq(
                userOutBalanceAfter,
                userOutBalanceBefore + amountOut,
                'User balance should increase by amount out'
            );

            // Verify remaining amount decreased appropriately
            uint256 remainingAfter = twapBot.remainingAmount(orderId);
            if (orderAfter.slicesExecuted < maxPossibleSlices) {
                // Not final slice - should decrease by slice size
                uint256 expectedSliceSize = orderBefore.sliceSize;
                assertEq(
                    remainingAfter,
                    remainingBefore - expectedSliceSize,
                    'Remaining should decrease by slice size'
                );
            } else {
                // Final slice - should be 0
                assertEq(
                    remainingAfter,
                    0,
                    'Final slice should clear remaining amount'
                );
            }

            // Verify next execution time updated (unless it's the final slice)
            if (orderAfter.slicesExecuted < maxPossibleSlices) {
                assertGt(
                    orderAfter.nextExecutionTime,
                    orderBefore.nextExecutionTime,
                    'Next execution time should advance'
                );
            }
        }
    }
